...
export function recomputeBehaviouralSnapshot(userId: string): number {
  ...
  // Every play counts here, parent picks included: this snapshot is the
  // watch weight (#217 keeps a parent pick's plays as ordinary watch weight).
  const rows = db.prepare(PERSON_AGGREGATE_SQL)
    .all(personAggregateParams(userId, { excludeParentPicks: false })) as AggregateRow[];
