...
**Why blur and muted colour:** We extract a representative frame from the video using ffmpeg's `thumbnail` filter (representative across a 300-frame window, seeking to the 25% point to avoid intros and end cards), then apply a heavy Gaussian blur (`gblur`) with reduced saturation and slight brightness reduction via ffmpeg's `eq` filter. The blur removes readable text, expressions, and fine-grained detail while preserving colour and compositional shape — enough for recognition ("the redstone one"), not enough to carry the engagement-engine signal. The saturation reduction keeps the palette calm without going greyscale. Any darkening for text legibility is handled by a CSS overlay in the PWA rather than baked into the image, so the overlay can be tuned independently of the image pipeline. The three knobs (`EDDY_THUMB_BLUR_SIGMA`, `EDDY_THUMB_SATURATION`, `EDDY_THUMB_BRIGHTNESS`) cover the whole aesthetic range; re-running `npm run thumbs:backfill` reskins the whole library without a schema change.
