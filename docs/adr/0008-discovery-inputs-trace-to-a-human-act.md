...
The pre-seeded catalogue is therefore no longer a *forced* inference target. The seed rows that existed **only** as inference targets (declared by nobody) and were the leak fodder — `news_analysis`, `programming`, `camping` — were removed (migration 037); declared seeds stay. The add-interest browse grid (`GET /interests/`) was already unused and was removed; declared interests are added freeform (`/user-add`) or by Keeping an inference.
