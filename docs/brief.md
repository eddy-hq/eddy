...
- **Rubric** — v1 written: `docs/guard-rubric.md`. Eight scored dimensions, four hard stops, two flags, a per-age-band limits table and the verdict mapping. 6a turns it into the guard module's versioned source, read by the guard prompt, parent reason chips, and (later) the classifier track. Models score dimensions; the limits table decides.
  Built: `src/modules/guard/rubric.ts` (`rubric-v1`, tested against the doc's tables) and a `candidate-v4` prompt — Gemma scores dimensions, hard stops and flags with one short reason, `verdictFromScores` decides per band; static-first prompt so Ollama reuses the cached rubric prefix; scores stored in `guard_eval.rubric_scores_json`. Behind `GUARD_CANDIDATE_PROMPT` (`v3` default, `v4`) for discovery and the second pass. Switched to `v4` in production on 2026-09-25 — modestly better on measurement, more testing to follow with `guard:rerun-parked -- --prompt v4 --sample N` (and `--population decided`).
...
- **Parent decision surface** in the PWA (not `/admin/guard-review` — "review" is a glossary avoid-term):
  ...
  Built (slice 1): `/decisions?userId=<parent>` in the PWA over `/parent/decisions`. Today shows recent escalations then the day's spot checks, with the spot checks keeping their slots inside the cap; catch-up works through either pile (escalations of any age, or spot-check batches drawn from all history). Parent-only by id (a kid's id is refused), on the same network posture as the other PWA routes.
  Built (slice 2): reason chips — one per rubric scored dimension, served with the queue from `rubric.ts`, plus a note of up to 280 characters — sit collapsed behind "Add a reason" on each card and go with the Allow / Block (or Block channel) request. Stored on `guard_decisions` (`reason_dimensions_json`, `reason_text`, migration 046); an unknown dimension is refused. Nothing reads them yet.
- **Daily nudge** via `notify()` — "N decisions waiting", only when the queue is non-empty. No per-item push in 6a.
  Built (slice 2): a `decisions_waiting` event to each parent, a count only, tapping through to `/decisions`. The count is Today's cards, from the same read as the queue. A BullMQ job scheduler on the M4, `DECISIONS_NUDGE_CRON` in `DECISIONS_NUDGE_TZ` (default 19:00 Europe/London); at most one a day per parent (`decision_nudges`). The iOS shell routes `/decisions` from this build on.
