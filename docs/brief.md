...
3. **Local ffmpeg frames.** The worker extracts 3 frames from the video file (`30%`, `50%`, `70%` of duration), Gemma scores their composition 0–10, and the best-composed frame that passes the floor is saved as `$THUMB_OUTPUT_PATH/{youtube_id}.webp` and served by nginx. An existing local frame from an earlier run is re-checked, not trusted.
4. **Neutral placeholder.** If nothing passes, the card gets a plain dark placeholder (`_neutral-placeholder.webp`, same directory) — never the creator thumbnail.
